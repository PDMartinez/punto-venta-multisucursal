...
var tablaCanales = $(".tablaCanales").DataTable({
  ...
  "ajax": "ajax/tablaCanales.ajax.php",
  "deferRender": true,
  "retrieve": true,
  "processing": true,
  "language": {
...
function guardarFormularioDescuento(){
  ...
  var cmbCanal = $("#idDescuento").val()+"/"+$("#tokenDescuento").val();

  var txtDescuento = $("#txtDescuento").val();

  var txtDesde = $("#txtDesde").val().replace(/\./g, '');

  var txtHasta = $("#txtHasta").val().replace(/\./g, '');
...
$(document).on("click", ".AsiganarCanal", function() {
  ...
  $("#idDescuento").val(id_canal);
  $("#tokenDescuento").val(token_canal);
   ...
   var datos = new FormData();
      ...
      $.ajax({
        ...
        for (var i = 0; i < respuesta.length ; i++) {
         ...
         <td class="text-center" >`+new Intl.NumberFormat().format(respuesta[i]["MONTO_DESDE"])+' Gs.'+`</td>
         <td class="text-center">`+new Intl.NumberFormat().format(respuesta[i]["MONTO_HASTA"])+' Gs.'+`</td>
