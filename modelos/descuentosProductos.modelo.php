<?php 
	...
	class ModeloDescuentosProductos{
		...
		static public function mdlIngresarDescuento($tabla, $datos){
			...
			$stmt = Conexion::conectar()->prepare("SELECT * FROM $tabla WHERE (((:Desde >= 	CANTIDAD_DESDE AND :Desde <= CANTIDAD_HASTA) OR (:Hasta >=CANTIDAD_DESDE AND :Hasta <= CANTIDAD_HASTA) OR (:Hasta >= CANTIDAD_HASTA AND :Desde <=CANTIDAD_DESDE)) AND (COD_CANAL = :cod_canal) AND(TOKEN != :Token) )");
			...
			if($cuenta <= 0){
				...
				$stmt=Conexion::conectar()->prepare("INSERT INTO $tabla(COD_CANAL, DESC_CANAL, CANTIDAD_DESDE, CANTIDAD_HASTA, TOKEN,FECHA_REGISTRO) VALUES (:Canal, :Descuento, :Desde, :Hasta, :Token,:fecha_registro)");
		...
		static public function mdlEditarDescuento($tabla, $datos){
			...
			// if($datos["estado"] == 1){
				...
				$stmt = Conexion::conectar()->prepare("SELECT * FROM detcanal WHERE (((:Desde >= CANTIDAD_DESDE AND :Desde <= CANTIDAD_HASTA) OR (:Hasta >= CANTIDAD_DESDE AND :Hasta <= CANTIDAD_HASTA) OR (:Hasta >= CANTIDAD_HASTA AND :Desde <= CANTIDAD_DESDE)) AND (COD_CANAL = :cod_canal) AND(TOKEN != :Token) )");
			...
			if($cuenta <= 0){
				...
				$stmt = Conexion::conectar()->prepare("UPDATE $tabla SET COD_CANAL = :Canal, DESC_CANAL = :Descuento, CANTIDAD_DESDE = :Desde, CANTIDAD_HASTA = :Hasta, FECHA_MODIFICACION = :FechaMod WHERE TOKEN = :Token");
